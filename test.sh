...
if [ "$JSON_FILES_COUNT" -eq 3 ]; then
    rm test1.csv test2.csv test3.csv
    echo "Test passed: Found 3 JSON files in the output bucket."
else
    rm test1.csv test2.csv test3.csv
    echo "Test failed: Expected 3 JSON files, but found $JSON_FILES_COUNT."
    exit 1
fi
